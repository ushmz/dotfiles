...
function notify_precmd {
	...
	notif_status=$?
	if [ -n "${SLACK_NOTIFY_WEBHOOK_URL+x}" ] && [ -n "${SLACK_NOTIFY_USER_NAME+x}" ] && [ ${TTYIDLE} -gt ${SLACK_NOTIF_THRESHOLD:-180} ] && [ ${notif_status} -ne 128 ] && [ ${notif_status} -ne 129 ] && [ ${notif_status} -ne 130 ] && [ ${notif_status} -ne 146 ]; then
